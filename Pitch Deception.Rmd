...
```{r}
attach(MLBPitcher)
PitchModel <- rpart(launch_speed ~ spin_rate + velocity + release_extension + eff_min_vel + effective_speed, data = Pitch_train, method = 'class')
```

```{r}
baseImp <- varImp(PitchModel)
baseImp
...
ggplot(baseImp, aes(Overall, row.names(baseImp))) + 
  ...
  labs(title = "Deception Importance", x = "Importance", y = "Variable")
```

```{r}
DeceptionTrain <- predict(PitchModel, Pitch_train, type = 'vector')
DeceptionTest <- predict(PitchModel, Pitch_test, type = "vector")
```
